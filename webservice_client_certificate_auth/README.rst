...
Adds support for Client Side Certificates to the ``webservice_core``
module.
...
On the webservice backend, set the authentication type to "Client
Certificate" and fill in:

-  **Client Certificate Path:** path to the certificate file (or to a
   single file containing both the certificate and the private key).
-  **Client Private Key Path:** optional, leave empty if the private key
   is bundled in the certificate file.

To manage these paths with ``server_environment``, see
``webservice_client_certificate_auth_server_env``.

Usage
=====

When a call is made using the backend, the ``cert`` parameter is
automatically injected into the underlying Python ``requests`` call
